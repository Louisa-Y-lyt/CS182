...
class DynamicProgramming:
    ...
    def updated_action_values(self, state: State) -> np.ndarray:
        ...
        ##### YOUR CODE HERE #####
        ##########################
        if self.terminal_states[state]: z_i = np.zeros(4)
        else: 
            z_i = []
            for ai, next_states in self.P[state].items():
                z = []
                for possible_s in next_states:
                    z.append(possible_s[0]*self.V[possible_s[1]])
                z_i.append(sum(z)) 
        return np.array(z_i)
    ...
    def value_iteration(self):
        ...
        ##########################
        change = -1
        while change == -1 or change > self.epsilon*(1-self.gamma)/self.gamma:
            change = 0
            for state, action in self.P.items():
                V_new = self.rewards[state] + self.gamma * max(self.updated_action_values(state))
                if abs(self.V[state] - V_new) > change:
                    change = abs(self.V[state] - V_new)
                self.V[state] = V_new
            for state, action in self.P.items():
                if max(self.updated_action_values(state)) > self.updated_action_values(state)[self.policy[state]]:
...
class QLearning:
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if rnd < self.epsilon or max(self.Q[state]) == 0:
            a = self.env.action_space.sample()
        else: a = np.argmax(self.Q[state])
        return a
    ...
    def q_learning(self, num_episodes=10000, interval=1000, display=False, step_limit=10000):
        ...
        for e in range(1, num_episodes+1):
            ...
            while not finished and num_steps < step_limit:
                ...
                ##### YOUR CODE HERE #####
                ##########################
                self.Q[curr_state][action] = (1-alpha)*self.Q[curr_state][action] + alpha*(self.rewards[next_state]+self.gamma * max(self.Q[next_state]))
...
if __name__ == "__main__":
    ...
    ##### YOUR CODE HERE #####
    ##########################
    steps = num_steps_array
    counts, bins = np.histogram(steps)
    plt.hist(bins[:-1], bins, weights=counts)
    plt.title('the histogram of the number of steps an agent takes to reach the goal over 1000 episodes using the final policy')
    ...
    ##### YOUR CODE HERE #####
    ##########################
    result = my_policy.q_learning()
    plt.plot(list(range(1000, 10001, 1000)), result)
    plt.title('the mean returns over 100 episodes of the Q-learning agent every 1000 episodes')
    plt.xlabel('episodes')
...
if __name__ == "__main__":
    ...
    #### YOUR CODE HERE ####
    ##########################
    plt.scatter(X.loc[y==0,'petal_length'], X.loc[y==0,'petal_width'], c = 'blue', label='0')
    plt.scatter(X.loc[y==1,'petal_length'], X.loc[y==1,'petal_width'], c = 'orange', label='1')
    plt.xlabel('petal_length')
    plt.ylabel('petal_width')
    plt.title('scatter plot of the penal length and width of iris flowers')
    ...
    n = 100
    x1=np.linspace(np.min(X.petal_length)-0.1,np.max(X.petal_length)+0.1,n)
    x2=np.linspace(np.min(X.petal_width)-0.1,np.max(X.petal_width)+0.1,n)
    x1v, x2v = np.meshgrid(x1, x2)
    X_simul = np.c_[x1v.ravel(), x2v.ravel()]
    yhat1 = logit.predict(X_simul)
    ...
    for i in range(len(y)):
        if pred[i] != y[i]: miscount += 1
    acc = 1 - (miscount/len(pred))
    print(acc)
    # baseline model
    prop_1 = sum(y)/len(y)
    baseline_acc = max(prop_1, (1-prop_1))
    print(baseline_acc)
    improvement = (acc-baseline_acc)/baseline_acc
    print(improvement)
...
def perceptron_iter(X:pd.Series, y:float, weight_vector:np.array)->Tuple[np.array,bool]:
    ...
    ##### YOUR CODE HERE #####
    ##########################
    pred = np.dot(weight_vector, X.T).flatten()[0]
    if pred >= 0: th = 1
    elif pred < 0: th = -1
#     print(pred)
#     print(y*pred)
    if th != y:
        weight_vector = np.array(weight_vector + np.multiply(y,X)).flatten()
        change = True
#     print(weight_vector)
...
def run_perceptron_algo(X:pd.DataFrame, y:np.array, weight_vector:np.array, max_iter:int=1000)->Tuple[np.array,int]:
    ...
    while not converge and iterations < max_iter:
        ...
        changes = np.multiply(changes, 1)
        if sum(changes) == 0: converge = True

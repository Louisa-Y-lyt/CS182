...
class SudokuGameBoard:
    ...
    def get_related_coords(self, coord:tuple)->list:
        ...
        related = []
        for i in range(9):
            related.append((coord[0],i))
            related.append((i, coord[1]))
            
        square_x = int((coord[0]) / 3) * 3
        square_y = int((coord[1]) / 3) * 3

        for x in range(0, 3):
            for y in range(0, 3):
                related.append((square_x + x, square_y + y))

        related_set = list(set(related))
        related_set.remove(coord)
        return related_set
    ...
    def fwd_checking(self):
        ...
        #### YOUR CODE HERE ####
        old = {}
        while self.coordinate_dict != old:
            old = deepcopy(self.coordinate_dict)
            for v in self.coordinate_dict:
                if len(self.coordinate_dict[v]) == 0:
                        return False
                if len(self.coordinate_dict[v]) == 1:
                    for vOther in self.get_related_coords(v):
                        if vOther not in self.coordinate_dict: continue
                        if vOther in self.coordinate_dict:
                            if self.coordinate_dict[v][0] in self.coordinate_dict[vOther]:
                                self.coordinate_dict[vOther].remove(self.coordinate_dict[v][0])                   
...
def MRV_heuristic(coordinate_dict:dict)->tuple:
    ...
    var = None
    mrv = float('inf')
    for v in coordinate_dict:
        if len(coordinate_dict[v]) < mrv:
            var = v
            mrv = len(coordinate_dict[v])
    return var
...
def Sudoku_Solver_IP(starting_board: np.array):
    ...
    for row in rows:
      for col in cols:
        list1 = []
        for k in values:
          list1.append(decision_variables[k][row,col])
        constraints.append(sum(list1)>=1)
        constraints.append(sum(list1)<=1)
    ...
    for row in rows:
      for k in values:
        list2 = []
        for col in cols:
          list2.append(decision_variables[k][row,col])
        constraints.append(sum(list2)>=1)
        constraints.append(sum(list2)<=1)
    ...
    for col in cols:
      for k in values:
        list3 = []
        for row in rows:
          list3.append(decision_variables[k][row,col])
        constraints.append(sum(list3)>=1)
        constraints.append(sum(list3)<=1)
    ...
    for grid in grids:
        ...
        for k in values:
          list4 = []
          for row in range(3*grid_row, 3*(grid_row+1)):
            for col in range(3*grid_col, 3*(grid_col+1)):
              list4.append(decision_variables[k][row,col])
          constraints.append(sum(list4)>=1)
          constraints.append(sum(list4)<=1)
    ...
    for row in rows:
      for col in cols:
        if(starting_board[row][col] != 0):
          k = starting_board[row][col]-1
          constraints.append(decision_variables[k][row,col] >= 1)
          constraints.append(decision_variables[k][row,col] <= 1)
    ...
    for k in values:
      for row in rows:
        for col in cols:
          list5.append(decision_variables[k][row,col])
    objective = cp.Maximize(sum(list5))#### YOUR CODE HERE #### Replace 1 with an actual objective
...
def Min_Makespan_Solver(task_times:list, K:int)->tuple:
    ...
    #### YOUR CODE HERE ####    
    ### IP Solver ###
    constraints, objective, assignment = Min_Makespan_IP(task_times, K) # Pass in the starting board into the IP Sudoku solver
    IP_obj = cp.Problem(objective, constraints) 
    a = IP_obj.solve(solver=cp.GLPK_MI, verbose=False) # Solve the IP using cvxpy
    print(a)
    return (a, assignment[0].value)

def Min_Makespan_IP(task_times:list, K:int)->tuple:
    # return a tuple with three elements: a list of constraints, an objective function, and a decision variable
    tasks_amount = len(task_times)
    decision_variables = [cp.Variable((tasks_amount,K),boolean=True), cp.Variable()]
    constraints=[]

    machines = range(K)
    tasks = range(tasks_amount)

    for j in machines:
      list1 = []
      for i in tasks:
        list1.append(task_times[i]*decision_variables[0][i,j])
      d = sum(list1)
      constraints.append(d<=decision_variables[1])
     
    for i in tasks:
      list2 = []
      for j in machines:
        list2.append(decision_variables[0][i,j])
      constraints.append(sum(list2)<=1)
      constraints.append(sum(list2)>=1)
    
    objective = cp.Minimize(decision_variables[1])
